...
국내주식 **182개 REST 엔드포인트** + **조건검색 4종**(WebSocket) + **19종 실시간 데이터**.
메서드 이름과 API ID 는 키움 공식 가이드 기준입니다.

## 인증

```python
api.login()      # 접근토큰 발급 (선택 — 첫 호출에서 자동 발급됨)
api.logout()     # 접근토큰 폐기
```

## 연속조회

모든 REST 메서드는 `cont_yn` / `next_key` 를 받습니다.
